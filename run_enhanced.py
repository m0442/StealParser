#!/usr/bin/env python3
"""
Enhanced Leaked Data Parser - Main Execution Script
Run this file to start the parser application with improved error handling
"""
...
class EnhancedParserGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced Leaked Data Parser & Analyzer v2.1.0")
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = tk.Label(main_frame, text="Enhanced Leaked Data Parser & Analyzer", 
                              font=('Arial', 22, 'bold'), fg='#4CAF50', bg='#2b2b2b')
        title_label.grid(row=0, column=0, columnspan=3, pady=(0, 20))
        
        # Version info
        version_label = tk.Label(main_frame, text="Version 2.1.0 - Enhanced Error Handling", 
        ...
        ttk.Label(main_frame, text="Export Format:").grid(row=4, column=0, sticky=tk.W, pady=5)
        self.format_var = tk.StringVar(value="json")
        format_combo = ttk.Combobox(main_frame, textvariable=self.format_var, 
                                   values=["json", "csv", "excel", "html", "pdf"], state="readonly")
        format_combo.grid(row=4, column=1, sticky=(tk.W, tk.E), padx=(10, 5), pady=5)
        
        # Parse button
        self.parse_btn = ttk.Button(main_frame, text="Start Enhanced Parsing", command=self.start_parsing)
        self.parse_btn.grid(row=5, column=0, columnspan=3, pady=20)
        ...
        # Status text
        self.status_text = tk.Text(main_frame, height=20, width=90, bg='#3c3c3c', fg='#ffffff')
        ...
        # Initial message
        self.log_message("🚀 Enhanced Leaked Data Parser v2.1.0 loaded successfully!")
        self.log_message("✅ All systems ready for data parsing")
    ...
    def parse_data(self, input_dir, output_dir, export_format):
        try:
            self.log_message("🔍 Starting enhanced data parsing...")
            ...
            output_filename = f"enhanced_parsed_data_{timestamp}.{export_format}"
            ...
            # تصدير التقرير
            analyzer.export_analysis_report('detailed_analysis.json')
